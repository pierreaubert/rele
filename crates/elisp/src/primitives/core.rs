...
pub fn add_primitives(interp: &mut crate::eval::Interpreter) {
    ...
    for name in [
        "eq", "eql", "equal", "not", "null", "symbolp", "numberp",
        "listp", "consp", "stringp", "atom", "integerp", "floatp",
        "zerop", "natnump", "functionp", "subrp", "sequencep",
        "char-or-string-p", "booleanp", "keywordp", "obarrayp",
        "characterp", "arrayp", "nlistp", "byte-code-function-p",
        "hash-table-p",
    ] {
        interp.define(name, LispObject::primitive(name));
    }
    for name in [
        "princ", "prin1", "string=", "string<", "concat", "substring",
        "string-to-number", "number-to-string", "make-string",
        "upcase", "downcase", "capitalize", "safe-length",
        "string", "regexp-quote", "max-char",
        "string-replace", "string-trim", "string-prefix-p",
        "string-suffix-p", "string-join", "char-to-string",
        "string-to-char", "string-width", "multibyte-string-p",
        "string-search", "string-equal", "string-lessp",
        "compare-strings", "split-string",
        "substring-no-properties",
        "key-description", "upcase-initials",
        "unibyte-string", "string-lines",
        "format-prompt", "string-bytes",
        "decode-char", "encode-char",
    ] {
        interp.define(name, LispObject::primitive(name));
    }
    for name in [
        "aref", "aset", "make-vector", "vconcat", "vectorp",
        "make-bool-vector", "bool-vector",
    ] {
        interp.define(name, LispObject::primitive(name));
    }
    for name in [
        "elt", "copy-alist", "plist-get", "plist-put", "plist-member",
        "remove", "remq", "number-sequence",
        "proper-list-p", "delete", "rassq", "rassoc",
        "take", "ntake", "length<", "length>", "length=",
        "fillarray", "memql",
    ] {
        interp.define(name, LispObject::primitive(name));
    }
    for name in [
        "record", "recordp", "record-type",
    ] {
        interp.define(name, LispObject::primitive(name));
    }
    for name in [
        "make-sparse-keymap", "make-keymap", "keymapp", "define-key",
        "type-of",
    ] {
        interp.define(name, LispObject::primitive(name));
    }
    for name in [
        "identity", "ignore", "rele--rx-translate",
        "autoload-do-load", "autoload-compute-prefixes",
        "load-history-filename-element", "format-message",
    ] {
        interp.define(name, LispObject::primitive(name));
    }
    ...
    let alias_to_ignore = [
        ...
        "obarray-make", "obarray-get", "obarray-put",
        "optimize-char-table", "make-char-table",
        "set-char-table-parent",
        "standard-case-table", "standard-syntax-table",
        "syntax-table", "set-syntax-table",
        "char-table-extra-slot", "char-table-range",
        "unify-charset", "find-file-name-handler",
        "unicode-property-table-internal",
        "set-char-table-range", "set-char-table-extra-slot",
        "map-char-table", "modify-category-entry",
        "modify-syntax-entry", "set-category-table",
        "define-category", "set-case-syntax",
        "set-case-syntax-pair", "set-case-syntax-delims",
        "add-variable-watcher", "remove-variable-watcher",
        "get-variable-watchers", "variable-binding-locus",
        "interactive-form", "command-modes",
        "symbol-with-pos-pos",
        "native-comp-unit-file", "native-comp-unit-set-file",
        "subr-native-comp-unit", "subr-native-lambda-list",
        "subr-type",
        "base64-encode-string", "base64-decode-string",
        "base64-encode-region", "base64-decode-region",
        "base64url-encode-string", "base64url-encode-region",
        "secure-hash", "secure-hash-algorithms", "md5",
        "buffer-hash", "locale-info", "load-average",
        "buffer-line-statistics", "clear-string",
    ...
    let phase8_predicates = [
        "bufferp", "markerp", "closurep", "interpreted-function-p",
        "threadp", "mutexp", "condition-variable-p", "user-ptrp",
        "module-function-p", "native-comp-function-p",
        "integer-or-marker-p", "number-or-marker-p",
        "vector-or-char-table-p", "bare-symbol-p",
        "symbol-with-pos-p", "bool-vector-p",
    ];
    ...
    let stub_names = [
        "obarrayp",
        "make-bool-vector", "bool-vector",
        "window-minibuffer-p", "frame-internal-border-width",
        "image-type-available-p", "gnutls-available-p",
        "libxml-available-p", "dbus-available-p",
        "native-comp-available-p", "display-graphic-p",
        "display-multi-frame-p", "display-color-p", "display-mouse-p",
        "get-char-property", "get-pos-property",
        "get-text-property", "text-properties-at",
        "last-nonminibuffer-frame", "tty-top-frame",
        "accessible-keymaps",
        "documentation", "documentation-property",
        "backward-prefix-chars", "undo-boundary",
        "buffer-text-pixel-size", "window-text-pixel-size",
        "make-category-table", "category-table",
        "coding-system-plist", "coding-system-p", "coding-system-aliases",
        "get-load-suffixes", "get-truename-buffer",
        "backtrace-frame--internal",
        "make-network-process",
        "make-process", "make-serial-process", "make-pipe-process",
        "call-process", "call-process-region", "start-process",
        "open-network-stream", "process-list", "get-process",
        "process-status", "process-attributes",
        "execute-kbd-macro", "current-input-mode",
        "make-indirect-buffer", "insert-buffer-substring",
        "insert-buffer-substring-no-properties",
        "window-system", "redisplay", "force-mode-line-update",
        "frame-visible-p", "frame-live-p", "frame-parameters",
        "window-live-p", "minibufferp", "minibuffer-window",
        "minibuffer-depth", "recursion-depth",
        "current-message", "input-pending-p",
        "this-command-keys", "this-command-keys-vector", "recent-keys",
        "terminal-live-p", "terminal-list",
        "mark-marker", "mark", "marker-insertion-type",
        "marker-position", "set-marker", "copy-marker", "make-marker",
        ...
        "set-text-properties", "add-text-properties",
        ...
        "user-uid", "user-real-uid", "group-gid", "group-real-gid",
        "set-default-toplevel-value",
        "detect-coding-string", "detect-coding-region",
        "find-coding-systems-string", "find-coding-systems-region",
        "find-coding-systems-for-charsets",
        "coding-system-base", "coding-system-change-eol-conversion",
        "command-remapping", "buffer-swap-text",
        "color-values-from-color-spec", "color-values", "color-name-to-rgb",
        "coding-system-priority-list",
        "set-coding-system-priority", "prefer-coding-system",
        "keyboard-coding-system", "terminal-coding-system",
        "set-terminal-coding-system", "set-keyboard-coding-system",
        "completing-read", "read-from-minibuffer", "read-string",
        "read-number", "read-buffer", "read-file-name",
        "yes-or-no-p", "y-or-n-p", "message-box", "ding", "beep",
        "char-displayable-p", "char-charset", "char-category-set",
        "char-syntax", "syntax-class-to-char", "syntax-after",
        ...
        "overlay-properties", "overlay-get", "overlay-put",
        "overlays-at", "overlays-in", "overlayp",
        ...
        "image-frame-cache-size", "image-metadata", "create-image",
        "face-list", "face-attribute", "face-all-attributes",
        "face-bold-p", "face-italic-p", "face-font",
        "face-background", "face-foreground",
        "internal-lisp-face-p", "internal-lisp-face-equal-p",
        "set-face-attribute", "make-face", "copy-face",
        "menu-bar-lines", "menu-bar-mode", "tool-bar-mode",
        "tab-bar-mode", "popup-menu", "x-popup-menu",
        "mapatoms", "unintern", "symbol-plist",
        "add-to-list", "add-to-ordered-list",
        "remove-from-invisibility-spec", "add-to-invisibility-spec",
        "buffer-list", "buffer-base-buffer",
        ...
        "window-old-point", "window-old-pixel-height",
        ...
        "window-absolute-pixel-position", "window-point",
        "window-start", "window-end", "window-prompt",
        ...
        "frame-terminal", "frame-tool-bar-lines", "frame-menu-bar-lines",
        ...
        "set-face-underline", "set-face-strike-through",
        "frame-parameter",
        ...
        "string-pixel-width",
        "text-char-description", "single-key-description",
        ...
        "current-idle-time",
        "timer-set-time", "timer-set-function",
        "timer-set-idle-time", "timer-inc-time",
        ...
        "mutex-lock", "mutex-unlock", "mutex-name",
        ...
        "posn-image", "posn-object-width-height",
        "kbd", "global-set-key", "local-set-key",
        "global-unset-key", "local-unset-key",
        "define-key-after", "substitute-key-definition",
        "where-is-internal", "set-keymap-parent", "copy-keymap",
        "keymap-parent", "current-global-map", "current-active-maps",
        "use-global-map", "lookup-key", "map-keymap",
        "map-keymap-internal", "keyboard-translate",
        "keyboard-quit", "abort-minibuffers",
        "minibuffer-message", "set-quit-char",
        "kill-all-local-variables", "buffer-local-variables",
        "generate-new-buffer",
        "set-file-modes", "symbol-file",
        ...
        "string-to-syntax",
        "current-input-method", "activate-input-method",
        "deactivate-input-method", "set-input-method",
        "toggle-input-method", "describe-input-method",
        "syntax-table-p",
        ...
        "copy-abbrev-table", "define-abbrev",
        "number-sequence",
        ...
        "kill-new", "kill-append", "yank", "yank-pop", "current-kill",
        "define-fringe-bitmap", "set-fringe-bitmap-face",
        "fringe-bitmaps-at-pos", "fringe-bitmap-p",
        ...
        "debug-on-entry", "cancel-debug-on-entry", "debug", "mapbacktrace",
        ...
        "treesit-language-available-p", "treesit-library-abi-version",
